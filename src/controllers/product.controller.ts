...
const addProduct = async (req: Request<{}, IProduct>, res: Response) => {
    ...
    try {
        const newProduct = await productService.add({ productName, productPrice })
        if (!newProduct) {
...
const updateProductById = async (req: Request<{ id: string }, Partial<IProduct>>, res: Response) => {
    const { productName, productPrice } = req.body
    try {
        const updatedProduct = await productService.update(req.params.id, { productName, productPrice })
        if (!updatedProduct) {
            res.status(404).json({ message: "Product not found!" })
            return
        }
